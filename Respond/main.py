...
import plot, utils, data, clustering, constants 
import warnings
import random
...
maf_df = data.load_maf_data(file_path=constants.maf_file_path)
expression_df = data.load_txt_file_into_dataframe(file_path=constants.expression_file_path)
expression_df_melted = data.reformat_expression_data(df=expression_df)
mutation_expression_df_melted = data.preprocess_and_combine_mutation_expression(maf_df= maf_df, expression_df = expression_df_melted)
...
for target_gene in constants.genes:
    ...
    heatmap_data = data.generate_expression_heatmap(expression_df=expression_df, volcano_plot_df=volcano_plot_df, n=100)

    # cluster top 100 genes and samples
    row_linkage, col_linkage, row_clusters, col_clusters, gene_sil_score, sample_sil_score = clustering.hierarchical_clustering(expression_df_heatmap=heatmap_data, 
                                                                                                output_folder=target_gene,
                                                                                                row_threshold=7, col_threshold=7)
